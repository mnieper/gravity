...
given by
\[
  \phi|G\colon G \to M, p \mapsto \phi(p)
\]
...
\begin{thm}
  ...
  Every $p \in M$ possesses a neighborhood $G \in \mathfrak U^0(p, M)$ such that there
...
\begin{proof}[Proof of \prettyref{lem:paracompact_manifold}]
  ...
  $K$ is covered by $(U_p)_{p \in A}$. By construction, the function
...
\begin{proof}[Proof of \prettyref{thm:paracompact_manifold}]
  ...
  By construction, $\supp \lambda_i \subseteq \bigcup_{j \in J_i} V_j = U'_i \subseteq
...
\begin{cor}
  ...
  such that $\widehat \phi$ coincides with $\phi$ on a neighborhood $U \subseteq G$
  of $p$ in $M$.
\end{cor}
...
\begin{proof}
  ...
  \[
    \hat\phi\colon M \to \set R, p \mapsto \begin{cases}
...
neighborhood $G \in \mathfrak U^0(p, M)$ with $q \notin G$. Extending the
...
expect that $\alpha(t)$ moves infinitesimally to $\alpha(t) + \diff \alpha(t)$. 
In other words, we expect $\frac{\diff\alpha(t)}{\diff t}$ to be a vector on $M$
tangent to the curve $\alpha(t)$. At the moment, however, we have no notion of
...
function $\phi$. Thus such a vector would give rise to a function
\[
...
\begin{proof}
  ...
  \[
    \phi = \phi(p) + \sum_{i = 1}^n (x_i - x_i(p)) \, g_i
  \]
  ...
  \[
    \forall v \in \mathrm T_p M: v = \sum_{i = 1}^n (v \cdot x_i) \, \frac\partial{\partial x_i}|_p,
  \] 
...
\[
  (f_* v) \cdot (\phi \, \psi) = (v \cdot \phi) \, \psi(f(p)) +
    \phi(f(p)) \, (v \cdot \psi).
\]
...
\[
  ...
  \diff_p x_i(\frac{\partial}{\partial x_j}|_p) = \updelta_{ij}.
\]
...
\[
  \begin{aligned}
    L^*\colon & (\mathrm T_p^* M)^k \times (\mathrm T_p M)^\ell \to \set R, \\
    & (\alpha_0, \dotsc, \alpha_{k - 1}, v_1, \dotsc, v_\ell) \mapsto
    \alpha_0(L(\alpha_1, \dotsc, \alpha_{k - 1}, v_1, \dotsc, v_\ell),
  \end{aligned}
\]
that is a tensor of type $(k, \ell)$, and $L$ can be recovered by the formula
\[
...
\begin{thm}
  Let $M$ be a manifold and $X \in \mathfrak X(M)$. Then the following holds:
  ...
  following sense: If $\widetilde\alpha\colon \widetilde J \to M$ is any
  integral
  ...
  $\beta\colon -s + J \to M, t \mapsto \alpha(t + s)$ is again an integral curve.
...
\begin{proof}
  ...
  \end{align}
  By the Picard-Lindel\"of theorem, this ordinary differential equation has
  ...
  such that $\Phi|U_\epsilon(t) \times U\colon U_\epsilon(t) \times U \to M$
  ...
  the construction of \prettyref{eq:ode}. By the existence theorem
  for smooth local flows, \prettyref{thm:diff_flow}, there exists an
  $\epsilon \in \set R_+$ and a $\widetilde V \in \mathfrak U^0(x(p_0), G)$
  and a smooth function $\Psi\colon U_\epsilon(0) \times \widetilde V \to
  ...
  Let $t$, $s \in \set R$, $p \in D_s$. With $q \coloneqq \Phi_s(p)$ one has
  \[
    q \in D_t \iff t \in J_q = -s + J_p \iff t + s \in J_p \iff p \in D_{t + s}.
  \]
  ...
  The last set-theoretic property stated in that paragraph and that remains to
  ...
  $\Phi(p) \in U$. By \prettyref{eq:flow_proof}, $(t - s, \Phi_s(p)) \in D$,
  ...
  \]
  As a composition of morphisms, $\Phi|U_\epsilon(\tau) \times V$ is a 
...
\begin{xca}
  ...
  Prove that
  \[
    f_* v\colon \mathcal C^\infty(N) \to \mathcal \set R,
